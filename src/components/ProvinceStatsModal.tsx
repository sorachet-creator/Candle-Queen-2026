...
const regionNames: Record<Region, {
  TH: string;
  EN: string;
  CN: string;
}> = {
...
const ProvinceStatsModal = ({
  isOpen,
  onClose
}: ProvinceStatsModalProps) => {
  ...
  const filteredProvinces = useMemo(() => {
    return provinceData.filter(province => province.name[language].toLowerCase().includes(searchTerm.toLowerCase())).sort((a, b) => b.count - a.count);
  }, [searchTerm, language]);
  ...
  return <AnimatePresence>
      ...
      opacity: 0
          ...
          {/* Backdrop */}
          <motion.div className="absolute inset-0 bg-background/90 backdrop-blur-md" onClick={onClose} />

          {/* Modal */}
          <motion.div className="relative z-10 w-full max-w-2xl max-h-[80vh] bg-card border border-border rounded-2xl overflow-hidden" initial={{
        scale: 0.9,
        opacity: 0,
        y: 20
      }} animate={{
        ...
        duration: 0.3
            ...
            <div className="sticky top-0 bg-card border-b border-border p-6 z-10">
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={onClose} className="p-2 text-foreground/50 hover:text-foreground transition-colors">
                  <X className="w-6 h-6" />
                </button>
              </div>
              ...
              <div className="mb-4">
                ...
                const regionTotals = [{
                  ...
                  name: regionNames.international,
                  count: internationalData.count,
                  topProvince: null
                }, {
                  region: 'central' as Region,
                  name: regionNames.central,
                  count: provinceData.filter(p => p.region === 'central').reduce((s, p) => s + p.count, 0),
                  topProvince: provinceData.filter(p => p.region === 'central').sort((a, b) => b.count - a.count)[0]
                }, {
                  region: 'north' as Region,
                  name: regionNames.north,
                  count: provinceData.filter(p => p.region === 'north').reduce((s, p) => s + p.count, 0),
                  topProvince: provinceData.filter(p => p.region === 'north').sort((a, b) => b.count - a.count)[0]
                }, {
                  region: 'northeast' as Region,
                  name: regionNames.northeast,
                  count: provinceData.filter(p => p.region === 'northeast').reduce((s, p) => s + p.count, 0),
                  topProvince: provinceData.filter(p => p.region === 'northeast').sort((a, b) => b.count - a.count)[0]
                }, {
                  region: 'east' as Region,
                  name: regionNames.east,
                  count: provinceData.filter(p => p.region === 'east').reduce((s, p) => s + p.count, 0),
                  topProvince: provinceData.filter(p => p.region === 'east').sort((a, b) => b.count - a.count)[0]
                }, {
                  region: 'south' as Region,
                  name: regionNames.south,
                  count: provinceData.filter(p => p.region === 'south').reduce((s, p) => s + p.count, 0),
                  topProvince: provinceData.filter(p => p.region === 'south').sort((a, b) => b.count - a.count)[0]
                ...
                return regionTotals.map((item, index) => <motion.div key={item.region} initial={{
                  opacity: 0,
                  y: 20,
                  scale: 0.8
                }} animate={{
                  ...
                  damping: 15
                    ...
                    ease: "easeInOut"
                        ...
                        {item.topProvince && <p className="text-[10px] text-muted-foreground mt-1 truncate">
              ...
              <div className="relative">
                ...
                <input type="text" placeholder={t.provinceStats.searchPlaceholder} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-3 bg-secondary border border-border rounded-xl text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-gold/50" />
              </div>
            </div>
            ...
            <div className="overflow-y-auto max-h-[400px] p-6">
              <div className="space-y-4">
                ...
                {showInternational && <div className="flex items-center justify-between p-4 bg-gradient-to-r from-gold/10 to-transparent border border-gold/30 rounded-xl">
              ...
              return <div key={region} className="border border-border/50 rounded-xl overflow-hidden">
                      {/* Region Header */}
                      <button onClick={() => toggleRegion(region)} className="w-full flex items-center justify-between p-4 bg-secondary/50 hover:bg-secondary/70 transition-colors">
                        <div className="flex items-center gap-3">
                    ...
                    duration: 0.2
                        ...
                        return <motion.div key={province.name.TH} className="flex items-center justify-between p-3 bg-secondary/30 rounded-lg hover:bg-secondary/50 transition-colors" initial={{
                          opacity: 0,
                          x: -10
                        }} animate={{
                          opacity: 1,
                ...
                {filteredProvinces.length === 0 && <p className="text-center text-muted-foreground py-8">
